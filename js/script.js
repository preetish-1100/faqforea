...
(function () {
  ...
  const frames = [];
  let s3ImagesLoaded = 0;

  for (let i = 1; i <= totalFrames; i++) {
    const img = new Image();
    const frameNum = i.toString().padStart(3, '0');
    // Frame naming format ezgif-frame-XXX.webp
    img.src = `images/fourkindsfacemask/ezgif-frame-${frameNum}.webp`;
    frames.push(img);
  ...
  function drawFrame(index) {
    ...
    // Set internal canvas resolution to match image for maximum sharpness
    if (s3Canvas.width !== frames[index].naturalWidth) {
      s3Canvas.width = frames[index].naturalWidth;
      s3Canvas.height = frames[index].naturalHeight;
    }
    
    s3Ctx.clearRect(0, 0, s3Canvas.width, s3Canvas.height);
    s3Ctx.drawImage(frames[index], 0, 0, s3Canvas.width, s3Canvas.height);
  }
  ...
  // Debounced resize to handle responsiveness
  let s3ResizeTimeout;
  window.addEventListener('resize', () => {
    clearTimeout(s3ResizeTimeout);
    s3ResizeTimeout = setTimeout(() => {
       const st = ScrollTrigger.getAll().find(t => t.vars.trigger === '#s3');
       if (st) {
          const frameIndex = Math.min(totalFrames - 1, Math.floor(st.progress * totalFrames));
  ...
  const hContainer = D.querySelector('.h-container');

  gsap.to(panels, {
    x: () => -(hContainer.scrollWidth - W.innerWidth),
    ease: 'none',
    scrollTrigger: {
      trigger: '#s4',
      pin: true,
      scrub: 1,
      end: () => '+=' + (hContainer.scrollWidth - W.innerWidth),
